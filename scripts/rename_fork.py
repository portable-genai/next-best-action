...
Rewrites the package name, the CLI entry point, the ``MKT_`` env-var prefix, and the
...
seed data, IdP wiring, region, consent regimes) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "next_best_action"
_OLD_CLI = "mkt-nba"
_OLD_ENV_PREFIX = "MKT_"
_OLD_RESOURCE = "next-best-action"
# In this repo the distribution / repo name IS the resource stem, so the dist and the
# resource are the same string; the default new dist is therefore the ``--resource``
# value, and the duplicate replacement is de-duplicated below.
_OLD_DIST = "next-best-action"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``MKT_`` when it prefixes an UPPER token
        # (MKT_NBA_PROFILE, MKT_VERTICAL), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-next-best-action"
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (seed data, consent, IdP, region).")
    return 0
