...
`next-best-action` owns no consent store. It asks sibling `marketing-compliance-gate` through the
...
`marketing-compliance-gate` URL. This keeps one legal answer in production while preserving an offline demonstration.
...
itself is the sibling `agent-guardrail-gateway`; this repo consumes it rather than re-implementing it.
...
order or decision from the same inputs. The enterprise WORM audit system is `agent-observability`; the
...
`model-quality-gate` system; this repo's gate mirrors its thresholds so merges are guarded locally, and
...
swap the regulator-reference column, and re-review with local counsel; the `next-best-action`-control column
...
across banking and online retail. Enterprise campaign planning (`campaign-planner`), creative production
(`creative-studio`), performance attribution (`performance-marketing-optimisation`) and financial-promotions governance (`marketing-compliance-gate`)
