...
// What was actually shipped was dead markup: without a nonce the browser blocks Next's inline
...
//
//   1. Every directive the policy is supposed to carry is actually present, and none is empty.
...
//
// Rule 3 is the one that matters, and it is the one a header assertion cannot express: the
// header is byte-identical in the working case and in the broken one. A statically prerendered
// page was built before the nonce existed, so it emits bare script tags while the header
// advertises a nonce, and because `'strict-dynamic'` disables the `'self'` fallback, that
// combination blocks strictly MORE than the unfixed policy did. Header and markup have to agree,
// and only the markup knows.
//
...
/** Directives whose ABSENCE is this fleet's original defect: a console that emitted only
 * `frame-ancestors` had no default-deny at all. */
