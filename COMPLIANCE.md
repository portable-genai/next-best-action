# COMPLIANCE: `next-best-action` Next-Best-Action Recommendations and Cross-Sell Engine

This maps every General Principle (P-01..P-13) and dependency rule (R1..R8) to a concrete
control in **this** repo. Where a principle does not apply to `next-best-action`, it is marked **n/a** with
the reason. `next-best-action` is the **only per-customer** marketing repo, so its data-protection, tenancy
and consent controls are load-bearing (contrast `market-intelligence`..`performance-marketing-optimisation`, which handle no customer PII).
...
| # | Principle | How `next-best-action` implements it | Evidence |
|---|-----------|----------------------|----------|
...
| **P-05** | Grounding over fine-tuning | Offer context is retrieved from the governed `enterprise-knowledge-base`; the model explains, it is not trained on customer data | `ports/knowledge_base.py`, `domain/recommendation_service.py` |
| **P-06** | Human-in-the-loop / maker-checker | Every `RecommendationSet` is `requires_human_review=True`; a human signs off before an offer is actioned, and consent-suppressed offers are never recommendable; the escalation is ROUTED to the `human-review-console` maker-checker console (rule R8), not left as a boolean | `domain/recommendation_service.py`, `ports/consent.py`, `ports/review_router.py`, `adapters/*/review_router.py` |
...
| **P-08** | Eval-gated promotion | Offline eval gate scores ranking / eligibility accuracy, consent-suppression correctness and PII safety; `model-quality-gate` at promotion | `eval/run_eval.py`, `ports/observability.py` (`EvaluationGatePort.gate`) |
| **P-09** | Defense in depth / zero trust | CMEK (named on the feature dataset AND again on every table in it, because it cascades in BigQuery and not in Terraform's model, where an undeclared block reads as a key removal and replaces the table), least-privilege IAM, private endpoints, a distinct agent identity; **fail-closed object authorization** (a customer outside the verified principal's tenant is denied); redact then screen twice | `infra/terraform/kms.tf`, `iam.tf`, `domain/recommendation_service.py` (cross-tenant deny), `agent/callbacks.py` |
...
| **P-13** | Fair, consented marketing (advertising compliance) | Every candidate channel is checked against `marketing-compliance-gate` through its versioned client contract; an unavailable or non-allow answer is suppressed, never recommended. Eligibility / suitability remains deterministic in `next-best-action` | `ports/consent.py`, `adapters/{local,gcp,onprem}/consent.py`, `domain/eligibility_service.py` |

---

## Dependency rules

`next-best-action`'s mandatory dependencies are **`agent-guardrail-gateway`, `enterprise-knowledge-base`, `agent-registry`, `model-quality-gate` (gate), `agent-observability` and `marketing-compliance-gate`** (see
...
| Rule | Requirement | How `next-best-action` satisfies it | Evidence |
|------|-------------|---------------------|----------|
| **R1** | Customer PII handling: `agent-guardrail-gateway` + DLP redaction | **Load-bearing.** The full safety pipeline runs on every recommendation: redact (Cloud DLP, with the shared `pii-kit` national-id detectors), screen INPUT, screen OUTPUT, and the model-boundary callback redacts + screens again. Customer PII never reaches the model, a span or the audit sink | `domain/recommendation_service.py`, `ports/safety.py`, `adapters/gcp/dlp_redaction.py`, `agent/callbacks.py` |
| **R2** | Audit to `agent-observability` | Every recommendation writes an immutable, already-redacted WORM `AuditEvent`; the `platform` adapter posts to `agent-observability` `/v1/audit` | `adapters/gcp/cloud_logging_audit.py`, `adapters/platform/remote_audit.py` |
| **R3** | Governed RAG via `enterprise-knowledge-base` | Offer / product context is retrieved via the `enterprise-knowledge-base` governed KB (`KnowledgeBasePort`) so the explanation is grounded | `ports/knowledge_base.py`, `adapters/platform/remote_knowledge_base.py` |
...
| **R6** | Validated by `architecture-validator` at intake | As a new project, `next-best-action` is validated by the `architecture-validator` intake validator externally. n/a in-repo | intake handled by `architecture-validator` externally |
| **R7** | Marketing compliance via `marketing-compliance-gate` | `next-best-action` makes per-customer offers, so it asks `marketing-compliance-gate` through `consent-preference-kit` for each candidate channel. The managed adapter has no BigQuery fallback and refuses an unconfigured store; local is a fictional stand-in behind the same wire contract | `ports/consent.py`, `adapters/{local,gcp,onprem}/consent.py`, `tests/unit/test_consent_port.py` |
| **R8** | Route `requires_human_review` to `human-review-console` | Every escalated `RecommendationSet` is submitted to the `human-review-console` Human-Review & Maker-Checker Console via the shared `review-kit` client (redact-before-wire: the internal customer key is pseudonymized and national ids / email / phone are masked, so no raw customer identifier reaches `human-review-console`). `local` enqueues to a transactional outbox so the routing path runs offline, `gcp`/`platform` submit over S2S to `human-review-console`'s service intake (`HUMAN_REVIEW_URL`); the verified principal's tenant is threaded onto the wire | `ports/review_router.py`, `adapters/{local,platform,onprem}/review_router.py`, `adapters/_review_payload.py` |
...
| `next-best-action` control | Reference regime | What a supervisor looks for |
|---|---|---|
...
counsel. The `next-best-action`-control column is stable across regulators; only the mapping changes. The
