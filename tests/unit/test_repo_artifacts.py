...
def test_every_lockfile_pins_each_commons_at_the_same_commit() -> None:
    ...
    An N-way comparison rather than a pair, because the repo now ships three lockfiles and the
    pair form did not survive the third: it unpacked the glob above into exactly two names and
    raised `ValueError: too many values to unpack` the moment `requirements-demo.lock` appeared
    beside them. The glob exists so that a new lockfile CANNOT escape these checks; a hardcoded
    arity one screen below it put the same blind spot straight back, in the file whose own
    docstring warns about it.

    Disagreement is always a bug, never a choice: every lockfile here is compiled from one
    `pyproject.toml`, so the only way two of them can pin different commits is that one was not
    recompiled. That is not cosmetic when the odd one out is the demo lock, because the browser
    job installs it ON TOP of the dev lock and would pull a kit backwards on every run.
    """
    by_file = {
...
def test_make_lock_regenerates_every_lockfile_the_repo_ships() -> None:
    ...
    it, so it was never committed at all and the suite skipped for want of it. A lockfile has
    to be named somewhere `make lock` reaches, or it is regenerated by hand or not at all.
    """
...
def test_the_demo_extra_is_pinned_exactly_and_compiled_into_a_lockfile() -> None:
    ...
    assert re.search(
        r"^playwright==", (REPO_ROOT / "requirements-demo.lock").read_text(encoding="utf-8"), re.M
