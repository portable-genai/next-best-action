...
chosen deliberately: the profile must actually be ``local`` AND ``MKT_NBA_PROFILE`` (or the
...
# The persona id is the suffix of ``source`` after the colon. Entitlement groups use this
# system's marketing / next-best-action domain, but the subjects and tenants (including the
# cross-tenant persona) mirror the reference so per-tenant authZ is demoable offline.
_PERSONAS: tuple[Principal, ...] = (
    Principal(
        subject="demo.analyst@bank.example",
        principals=("group:nba-analyst", "group:marketing"),
        tenant="demo-bank",
    ...
    Principal(
        ...
        principals=("group:nba-analyst", "group:marketing", "group:nba-approver"),
        tenant="demo-bank",
        assurance="local-demo",
...
class LocalPersonaIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "MKT_NBA_PROFILE is not set, so the local profile was inherited rather than "
                "chosen; the seeded dev personas grant a tenant identity with no authentication "
                "and are refused. Set MKT_NBA_PROFILE=local deliberately for a dev or demo run, "
                "or MKT_NBA_PROFILE=gcp for a real deployment."
            )
