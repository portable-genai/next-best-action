"""A2A registry adapter (AgentRegistryPort) — agent discovery and governance for D5 (A3).

Backs the domain ``AgentRegistryPort`` with an in-process, **A2A v1.0**-style registry of
:class:`AgentCard` objects. In a standalone deployment D5 registers its own card here and
...
# D5's own skills, surfaced on its AgentCard so peers / the registry can discover the
# governed next-best-action capabilities the system offers (generic across verticals).
_D5_SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="recommend",
        name="Next-best-action recommendations",
        description=(
            "Rank eligible, consented offers for a customer (banking) or shopper (online "
            "retail) by deterministic propensity x value, with provenance on every "
            "recommendation, across JP/AU/SG."
        ),
    ),
    AgentSkill(
        id="eligibility",
...
class A2ARegistryAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # Seed the registry with D5's own card so a standalone deployment is discoverable.
    ...
    def agent_card_dict(self, name: str | None = None) -> dict:
        """Return the ``/.well-known/agent-card.json`` body for ``name`` (default: D5's)."""
    ...
    def _self_name(self) -> str:
        return self._settings.agent_engine.display_name or "next-best-action"

    def _self_card(self) -> AgentCard:
        return AgentCard(
            name=self._self_name(),
            description=(
                "D5 Next-Best-Action: Recommendations and Cross-Sell — deterministic eligibility, "
                "consent and ranking over the offer catalog, generic across banking and "
                "online retail and the JP/AU/SG markets, with cited findings."
            ),
            url=f"https://next-best-action.{self._settings.region}.example/a2a",
            version="1.0.0",
            skills=_D5_SKILLS,
