"""Gen AI evaluation gate adapter (EvaluationGatePort) — the A4 promotion gate for D5.

Backs the domain ``EvaluationGatePort`` with the **Gen AI evaluation service**, accessed
through ``vertexai.Client(project, location).evals``. Over a golden dataset of
recommendation cases it scores the system on the metrics that matter for a cited
next-best-action system (recommendation groundedness, citation accuracy, eligibility
accuracy, review safety) and maps the result onto an :class:`EvalReport` whose ``passed``
flag gates promotion in CI/CD.
