"""ReviewRouterPort: the boundary that routes an escalated recommendation set to Hrz7 (rule R8).

Every :class:`RecommendationSet` is consequential decision-support and always requires human
review (maker-checker, P-06): the agent is the maker, a qualified operator is the checker who
disposes before any offer is surfaced to a customer. Rule R8 says a producer that sets
...
the S2S submission, and the adapter redacts the per-customer PII before the wire.
...
class ReviewRouterPort(Protocol):
    def route(self, result: RecommendationSet, *, maker: str, tenant: str = "") -> None:
        """Route an escalated recommendation set to Hrz7 (idempotent per customer is ideal)."""
        ...
